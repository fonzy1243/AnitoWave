...
void DescriptorLayoutBuilder::add_binding(uint32_t binding, vk::DescriptorType type) {
    vk::DescriptorSetLayoutBinding newbind = vk::DescriptorSetLayoutBinding()
    .setBinding(binding)
...
void DescriptorAllocator::init_pool(vk::Device device, uint32_t maxSets, std::span<PoolSizeRatio> poolRatios) {
    ...
    for (PoolSizeRatio ratio : poolRatios) {
        poolSizes.push_back(vk::DescriptorPoolSize().setType(ratio.type).setDescriptorCount(static_cast<uint32_t>(ratio.ratio * maxSets)));
    }
    ...
    vk::Result createRes = device.createDescriptorPool(&pool_info, nullptr, &pool);
...
vk::DescriptorSet DescriptorAllocator::allocate(vk::Device device, vk::DescriptorSetLayout layout) {
    ...
    vk::DescriptorSet ds;
    vk::Result createRes = device.allocateDescriptorSets(&allocInfo, &ds);
    if (createRes != vk::Result::eSuccess) {
...
void DescriptorWriter::write_image(int binding, vk::ImageView image, vk::Sampler sampler, vk::ImageLayout layout, vk::DescriptorType type) {
    vk::DescriptorImageInfo& info = imageInfos.emplace_back(vk::DescriptorImageInfo().setImageView(image).setSampler(sampler).setImageLayout(layout));
...
void DescriptorWriter::write_buffer(int binding, vk::Buffer buffer, size_t size, size_t offset, vk::DescriptorType type) {
    vk::DescriptorBufferInfo& info = bufferInfos.emplace_back(vk::DescriptorBufferInfo().setBuffer(buffer).setOffset(offset).setRange(size));
...
vk::DescriptorPool DescriptorAllocatorGrowable::create_pool(vk::Device device, uint32_t setCount, std::span<PoolSizeRatio> poolRatios) {
    std::vector<vk::DescriptorPoolSize> poolSizes;
    for (PoolSizeRatio ratio : poolRatios) {
        poolSizes.push_back(vk::DescriptorPoolSize().setType(ratio.type).setDescriptorCount(static_cast<uint32_t>(ratio.ratio * setCount)));
    }
    ...
    vk::DescriptorPool newPool;
    vk::Result createRes = device.createDescriptorPool(&pool_info, nullptr, &newPool);
    if (createRes != vk::Result::eSuccess) {
        fmt::println("Failed to create descriptor pool");
    }
...
vk::DescriptorSet DescriptorAllocatorGrowable::allocate(vk::Device device, vk::DescriptorSetLayout layout, void *pNext) {
    ...
    if (createRes == vk::Result::eErrorOutOfPoolMemory || createRes == vk::Result::eErrorFragmentedPool) {
        fullPools.push_back(poolToUse);

        poolToUse = get_pool(device);
        allocInfo.descriptorPool = poolToUse;

        createRes = device.allocateDescriptorSets(&allocInfo, &ds);
        if (createRes != vk::Result::eSuccess) {
            fmt::println("Failed to allocate descriptor set");
        }
    }
