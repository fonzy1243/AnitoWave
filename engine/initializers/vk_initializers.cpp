...
vk::SemaphoreCreateInfo vkinit::semaphore_create_info(vk::SemaphoreCreateFlags flags) {
    vk::SemaphoreCreateInfo info = vk::SemaphoreCreateInfo()
    .setPNext(nullptr)
    .setFlags(flags);
...
vk::CommandBufferSubmitInfo vkinit::command_buffer_submit_info(vk::CommandBuffer cmd) {
    vk::CommandBufferSubmitInfo info = vk::CommandBufferSubmitInfo()
    .setPNext(nullptr)
...
vk::DescriptorSetLayoutCreateInfo vkinit::descriptor_set_layout_create_info(vk::DescriptorSetLayoutBinding *bindings, uint32_t bindingCount) {
...
vk::DescriptorBufferInfo vkinit::buffer_info(vk::Buffer buffer, vk::DeviceSize offset, vk::DeviceSize range) {
    vk::DescriptorBufferInfo binfo = vk::DescriptorBufferInfo()
    .setBuffer(buffer)
    .setOffset(offset)
    .setRange(range);
    return binfo;
...
vk::ImageViewCreateInfo vkinit::imageview_create_info(vk::Format format, vk::Image image, vk::ImageAspectFlags aspectFlags) {
    ...
    info.subresourceRange.setAspectMask(aspectFlags);
